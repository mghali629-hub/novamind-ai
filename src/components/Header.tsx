  ...
  return (
    <header className="sticky top-0 z-40 bg-[#0B0F17]/90 backdrop-blur-xl border-b border-indigo-500/20 font-sans">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
        ...
        <nav className="hidden lg:flex items-center gap-4 text-xs font-medium text-slate-300 overflow-x-auto no-scrollbar [ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden py-2">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className={`transition-colors whitespace-nowrap py-1 ${pathname === link.href ? 'text-indigo-400 border-b-2 border-indigo-400 font-bold' : 'hover:text-white'}`}
            >
              {link.label}
            </Link>
          ))}
  ...
  return (
    <footer className="bg-[#050810] border-t border-indigo-500/20 py-12 text-slate-400 text-xs font-mono">
      ...
      <div className="max-w-7xl mx-auto px-4 mt-8 pt-6 border-t border-slate-900 text-center text-slate-500 text-[11px] font-sans">
        © 2026 NovaMind AI Labs. All rights reserved.
      </div>
